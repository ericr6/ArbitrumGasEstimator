...
def compute_l1_gas_and_fee(input_data_hex, gas_price_l1, overhead=1400, safety_factor=1.1):
    """
    Estimate L1 gas used and L1 fee based on calldata size and safety margins.
    """
...
def get_eth_price_usd():
    """
    Fetch current ETH price in USD with CoinGecko and Binance fallback.
    """
...
def main():
    ...
    if NETWORK == "arbitrum":
        ...
        RPC_URL_ETH = "https://ethereum.publicnode.com"  # Mainnet L1
    elif NETWORK == "bellecour":
        RPC_URL_L2 = "https://bellecour.iex.ec"
        RPC_URL_ETH = None  # Bellecour is zero-fee, no L1
    elif NETWORK == "arbitrum-sepolia":
        RPC_URL_L2 = "https://sepolia-rollup.arbitrum.io/rpc"
        RPC_URL_ETH = "https://sepolia.drpc.org"  # Sepolia L1
    else:
    ...
    # Connect to L1 if needed
    w3_eth = None
    if RPC_URL_ETH:
        w3_eth = Web3(Web3.HTTPProvider(RPC_URL_ETH))
        if not w3_eth.is_connected():
            raise Exception(f"Failed to connect to L1 RPC ({RPC_URL_ETH}).")
    ...
    L2_fee_eth = (gas_used_L2 * gas_price_L2) / 1e18 if NETWORK != "bellecour" else 0

    # ---------------------------
    # L1 GAS & FEE (only for arbitrum and arbitrum-sepolia)
    # ---------------------------
    if NETWORK == "bellecour":
        l1_gas_used_est = 0
        L1_fee_eth = 0
    else:
    ...
    # ---------------------------
    # ARBITRUM MAINNET ESTIMATION
    # ---------------------------
    if NETWORK in ["bellecour", "arbitrum-sepolia"]:
        arb_gas_price_L2 = get_gas_price_arbitrum()
        w3_eth_mainnet = Web3(Web3.HTTPProvider("https://ethereum.publicnode.com"))
        gas_price_L1_mainnet = w3_eth_mainnet.eth.gas_price

        L2_fee_eth_arb = (gas_used_L2 * arb_gas_price_L2) / 1e18
        L2_fee_usd_arb = L2_fee_eth_arb * eth_price_usd
        L1_fee_eth_arb = (l1_gas_used_est * gas_price_L1_mainnet) / 1e18
    ...
    if NETWORK == "bellecour":
        print("  - L2 Fee            : 0 ETH (zero-fee chain)")
    else:
    ...
    if NETWORK in ["bellecour", "arbitrum-sepolia"]:
        ...
        print(f"  - Gas Price (L1) Eth: {gas_price_L1_mainnet / 1e9:.2f} gwei")
